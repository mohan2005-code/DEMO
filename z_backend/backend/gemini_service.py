...
def generate_fallback_letter(
    ...
    if strategy_type == "hardship":
        ...
        # Calculate a reasonable settlement target between 30% to 50%
        ...
        suggested_rate = max(2.0, outstanding_amount * 0.05 / 12)  # dummy calculation or a lower rate (e.g. 5-7%)
        body = (
            f"I am writing to request a restructure of the interest rate on my {loan_name} account. "
            f"Currently, my outstanding balance is ${outstanding_amount:,.2f} with an interest rate of {outstanding_amount:,.1f}% (or applicable rate). "
...
def generate_negotiation_letter(
    ...
    """
    Generates a negotiation letter. Uses Gemini API if configured, otherwise falls back to a high-quality local generator.
    """
    if not HAS_GEMINI:
    ...
    prompt = (
        ...
        f"2. Clearly mention the financial details: income, expenses, and why the current EMI is not sustainable.\n"
